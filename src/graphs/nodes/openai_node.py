"""
OpenAI API 调用节点
并行分支节点：调用 OpenAI API 对问候消息进行分析
...
from graphs.state import OpenAINodeInput, OpenAINodeOutput

OPENAI_API_URL = "https://api.openai.com/v1/chat/completions"


def openai_node(
    state: OpenAINodeInput,
    config: RunnableConfig,
    runtime: Runtime[Context]
) -> OpenAINodeOutput:
    """
    title: OpenAI 分析
    desc: 调用 OpenAI API 分析问候消息的质量和风格，返回改进建议
    integrations: 
    """
    ctx = runtime.context
    ...
    payload: dict = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "你是一个问候语质量分析师。请用一句话评价以下问候语的质量，并给出一个改进词。格式：评价:xxx, 改进:xxx"
            },
            {
                "role": "user",
                "content": greeting
    ...
    return OpenAINodeOutput(openai_result=result.strip())