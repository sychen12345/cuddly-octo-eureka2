...
映射 Coze 低代码工作流中的开始节点、业务节点和结束节点：
...
class GraphInput(BaseModel):
    ...
    grok_api_key: str = Field(
        default="",
        repr=False,
        description="运行时由用户输入的 Grok API Key；仅用于节点调用，不会出现在输出中",
    )
    openai_api_key: str = Field(
        default="",
        repr=False,
        description="运行时由用户输入的 OpenAI API Key；仅用于节点调用，不会出现在输出中",
    )
...
# ============= 节点的输入输出定义 =============
class BenchmarkDemandNodeInput(BaseModel):
    """对标与需求挖掘节点的输入。"""
...
class BenchmarkDemandNodeOutput(BaseModel):
    """对标与需求挖掘节点的输出。"""
...
class TopicCardNodeInput(BaseModel):
    """选题库与图文卡片节点的输入。"""
...
class TopicCardNodeOutput(BaseModel):
    """选题库与图文卡片节点的输出。"""
